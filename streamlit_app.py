...
# Safe pmdarima Import (mirrors XGBoost pattern)
try:
...
# Dynamically build model list based on successful imports
MODELS = ['Gradient Boosting', 'Random Forest', 'ARIMA', 'SARIMA', 'Moving Average', 'Naïve Forecast']
...
def load_and_clean_data(filepath):
    ...
    # AGGRESSIVE CLEANING: Strip commas/dollars and force numeric on ALL columns
    for col in df.columns:
        if df[col].dtype == 'object' or df[col].dtype == 'string':
            df[col] = df[col].astype(str).str.replace(',', '', regex=False).str.replace('$', '', regex=False).str.strip()
        df[col] = pd.to_numeric(df[col], errors='coerce')
        
    # Handle infinite values created by division errors in raw data
    df.replace([np.inf, -np.inf], np.nan, inplace=True)
    
    # Forward-fill missing data, then fill remaining with 0
...
def train_and_predict(_train, _test, target_col, features, horizon=30):
    ...
    # Safe fallback value: 7-day rolling mean, never NaN
    _fallback_arima = float(y_train.tail(7).mean())
    if np.isnan(_fallback_arima) or np.isinf(_fallback_arima):
        _fallback_arima = float(y_train.dropna().iloc[-1]) if len(y_train.dropna()) > 0 else 0.0

    _fallback_sarima = float(y_train.tail(14).mean())
    if np.isnan(_fallback_sarima) or np.isinf(_fallback_sarima):
        _fallback_sarima = _fallback_arima

    # --- 6. ARIMA ---
    # Strategy: pmdarima auto_arima (if installed) → statsmodels ARIMA(1,1,1) → rolling-mean fallback
    try:
        if PMDARIMA_AVAILABLE:
            ...
            pred_arima = np.array(model_arima.predict(n_periods=len(y_test)), dtype=float)
        else:
            # Direct statsmodels ARIMA when pmdarima is missing on cloud
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                arima_fit = StatsARIMA(y_train, order=(1, 1, 1)).fit()
            pred_arima = np.array(arima_fit.forecast(steps=len(y_test)), dtype=float)

        if np.isnan(pred_arima).any() or np.isinf(pred_arima).any():
            raise ValueError("ARIMA returned NaN/Inf predictions")

        results['ARIMA'] = {'pred': pred_arima, 'model': None}

    except Exception:
        # Last resort: use 7-day rolling mean (guaranteed finite)
        results['ARIMA'] = {'pred': np.full(len(y_test), _fallback_arima, dtype=float), 'model': None}

    # --- 7. SARIMA ---
    # Strategy: pmdarima auto_arima seasonal → statsmodels SARIMAX(1,1,1)(1,0,0,7) → rolling-mean fallback
    try:
        if PMDARIMA_AVAILABLE:
            ...
            pred_sarima = np.array(model_sarima.predict(n_periods=len(y_test)), dtype=float)
        else:
            # Direct statsmodels SARIMAX when pmdarima is missing on cloud
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                sarima_fit = SARIMAX(
                    y_train,
            ...
            pred_sarima = np.array(sarima_fit.forecast(steps=len(y_test)), dtype=float)

        if np.isnan(pred_sarima).any() or np.isinf(pred_sarima).any():
            raise ValueError("SARIMA returned NaN/Inf predictions")

        results['SARIMA'] = {'pred': pred_sarima, 'model': None}

    except Exception:
        # Last resort: use 14-day rolling mean (guaranteed finite)
        results['SARIMA'] = {'pred': np.full(len(y_test), _fallback_sarima, dtype=float), 'model': None}
...
def create_plotly_ts(train, test, predictions, title):
    fig = go.Figure()
...
if uploaded_file:
    ...
    if df is not None and not df.empty:
        ...
        # Dataset size safety check
        if len(df) < 20:
            st.error("Dataset is too small to perform accurate forecasting. Please upload a dataset with at least 20 historical records.")
            st.stop()
        ...
        # -----------------------------------------
            ...
            # Ensure horizon doesn't exceed test dataset length
            max_horizon = len(test)
            horizon_opts = [7, 14, 30]
            valid_horizons = [h for h in horizon_opts if h <= max_horizon]
            
            if not valid_horizons:
                valid_horizons = [max_horizon]
                
            horizon = col_sel2.selectbox("Forecast Horizon", valid_horizons, index=len(valid_horizons)-1)
            ...
            m4.metric('Accuracy', f"{res['metrics']['Accuracy']:.2f}%")
        ...
        # -----------------------------------------
            ...
            if hasattr(model_obj, 'feature_importances_'):
                ...
                # Fixed layout vs trace error for Plotly
        ...
        # -----------------------------------------
            ...
            st.metric("Forecasted Population Change", f"{pct_change:+.2f}%")
        ...
        # -----------------------------------------
            ...
            csv_forecast = gb_res.to_csv(index=True).encode('utf-8')
            ...
            st.download_button(label="Download Cleaned Dataset (CSV)", data=df.to_csv(index=True).encode('utf-8'), file_name='cleaned_uac_data.csv', mime='text/csv')
